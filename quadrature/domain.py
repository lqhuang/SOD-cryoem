...
class FixedPlanarDomain():
    def __init__(self,pts,res):
        ...
        if pts.shape[0] == 1:
            ...
            sqdist = np.copy(self.get_sqdist_mat())
            np.fill_diagonal(sqdist, np.inf)
            self.pt_resolution = np.sqrt(sqdist.min(axis=1))
...
class FixedDirectionalDomain():
    ...
    def __eq__(self,other):
        return type(self) == type(other) and len(self) == len(other) \
                 and self.dim == other.dim \
                 and self.resolution == other.resolution \
                 and np.all(self.dirs == other.dirs)
