...
def doimage_test(data_dir, model_file, use_angular_correlation=True):
    ...
    for idx in range(kernel.minibatch['N_M']):
        ...
        workspace = None
        g = np.zeros_like(slices_sampled, dtype=np.complex64)
        ...
        if kernel.sampler_S is not None:
            like, (cphi_S,cphi_I,cphi_R), csigma2_est, ccorrelation, cpower, workspace = \
                py_objective_kernels.doimage_RIS(slices_sampled, \
                    S_sampled, envelope, \
                    rotc_sampled, rotd_sampled, \
                    log_W_S, log_W_I, log_W_R, \
                    sigma2, g, workspace)
