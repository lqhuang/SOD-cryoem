...
class FiniteRunningSum:
    ...
    def get_sum(self):
        ...
        if self.history_upds == 0:
            ...
            ret = (self.history_wsum * self.history_upds /
                   self.N_sum) * self.history_mean
    ...
    def get_sumsq(self):
        ...
        if self.history_upds == 0:
            ...
            ret = (self.history_w2sum * self.history_upds /
                   self.N_sum) * self.history_meansq
    ...
    def set_value(self, key, value, decay=None):
        ...
        if self.history_upds > 0 and key not in self.history_updated:
            self.history_upds -= 1
            if self.history_upds == 0:
                # This isn't strictly necessary, but it frees memory and
                # ensures consistency
                self.history_mean = None
                self.history_wsum = None
                self.history_updated = None
        ...
        if decay is not None and 0 < decay and decay < 1:
            if self.history_upds > 0:
                self.history_wsum *= decay
            self.w_sum *= decay
            self.val_sum *= decay
            # This technically makes the step O(data set)...
            self.prev_ws *= decay
            if self.second_order:
                if self.history_upds > 0:
                    self.history_w2sum *= decay
                self.val2_sum *= decay
                self.w2_sum *= decay
                # This technically makes the step O(data set)...
                self.prev_w2s *= decay
