...
def readctfstxt(parfile, num=None):
    ...
    d['ANGAST'] = [np.pi * angast / 180.0 for angast in d['ANGAST']]
...
def readctfspar(parfile, num=None):
    ...
    fmt = ('I7', 'F8', 'F8', 'F8', 'F8', 'F8',
           'F8', 'I6', 'F9', 'F9', 'F8', 'F7')
    labels = ('IDX', 'PSI', 'THETA', 'PHI', 'SHX', 'SHY',
              'MAG', 'FILM', 'DF1', 'DF2', 'ANGAST', 'CCMAX')

    ls = [int(s[1]) for s in fmt]
    splits = np.concatenate([[0, ], np.cumsum(ls)])
    ...
    with open(parfile, 'r') as f:
        while num is None or len(data) < num:
            ...
            if l[0].lower() == 'c':
                continue
            data += [[fs[str.upper(fm[0])](l[s:e])
                      for fm, s, e in zip(fmt, splits[:-1], splits[1:])]]
...
class CTFStackBase:
    ...
    def set_datasign(self, dsign):
        for C in self.CTF:
            C.params['dscale'] = dsign * np.abs(C.params['dscale'])
    ...
    def write_defocus_txt(self, fname):
        with open(fname, 'wt') as f:
            ...
            for ctfI in self.CTFmap:
                ...
                df1 = cCTF.params['df1']
                df2 = cCTF.params['df2']
                angast = (180.0 / np.pi) * cCTF.params['angast']
    ...
    def __init__(self, parfile, mscope_params):
        ...
        if self.parfile[-3:].lower() == 'txt':
            with open(self.parfile, 'r') as f:
                numcols = max(len(f.readline().split()),
