...
def readMRC(fname, inc_header=False):
    """ Reads a MRC file. The header can be an option. Return a volume, (header) """
    ...
    with open(fname) as f:
        ...
        if datatype == 1:
            data = np.reshape(np.fromfile(f, dtype='int16', count=nx*ny*nz), \
                              (nx, ny, nz), order='F')
        if datatype == 2:
            data = np.reshape(np.fromfile(f, dtype='float32', count=nx*ny*nz), \
                              (nx, ny, nz), order='F')
    if inc_header:
        return data, hdr
    else:
...
def readMRCimgs(fname, idx, num = None):
    ...
    with open(fname) as f:
        ...
        if datatype == 1:
            return np.reshape(np.fromfile(f, dtype='int16', count=nx*ny*num), \
                              (nx, ny, num), order='F')
        if datatype == 2:
            return np.reshape(np.fromfile(f, dtype='float32', count=nx*ny*num), \
                              (nx, ny, num), order='F')

def readMRCheader(fname):
    """ Reads header of a MRC file. """
    hdr = None
    with open(fname) as f:
        hdr = {}
...
def animplot(fname):
    """ animated plot for a mrcs volume """
    a = readMRC(fname)
    plt.ion()
