...
class CombinedImageStack(ImageStackBase):
    def __init__(self, stacks):
        ...
        self.num_images = 0
        self.stack_idxs = np.empty((len(self.stacks), 2), dtype=np.uint32)
...
class CombinedImageStack_Iter:
    ...
    def next(self):
        ...
        if self.stkI == self.stack.stack_idxs.shape[0]:
            ...
            # get_image requires idx counting from 0, i.e. imgI - stack_idxs[i,
            # 0]
...
class FourierStack:
    ...
    def set_transform(self, premult, zeropad):
        ...
        self.fspacesum = np.zeros(2 * (self.get_num_pixels(),),
                                  dtype=density.complex_t)
        self.powersum = np.zeros(2 * (self.get_num_pixels(),),
